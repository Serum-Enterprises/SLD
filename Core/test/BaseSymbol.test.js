...
describe('Testing BaseSymbol', () => {
	test('Testing constructor', () => {
		...
		expect(() => new BaseSymbol('TEST', null, 123)).toThrow(new RangeError('Expected type to be one of "STRING", "REGEXP", "RULESET"'));
	...
	test('Testing get type', () => {
		...
		expect((new BaseSymbol('REGEXP', 'Hello World', 'test')).type).toBe('REGEXP');
		expect((new BaseSymbol('RULESET', 'Hello World', 'test')).type).toBe('RULESET');
	});
	...
	test('Testing set type', () => {
		...
		expect(() => baseSymbol.type = 'TEST').toThrow(new RangeError('Expected type to be one of "STRING", "REGEXP", "RULESET"'));
	...
	test('Testing debug', () => {
		expect((new BaseSymbol('STRING', 'Hello World', 'test')).debug()).toEqual({ type: 'STRING', value: 'Hello World', name: 'test' });
	})
});